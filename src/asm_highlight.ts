...
export class AsmHighlight {
     ...
     * @returns HTML string
     */
    static toHTML(assemblySource: string, addDivLine: boolean = true): string {
        var iFound: boolean, fFound: boolean; //instruction Found, function Found
        var i: number, j: number, k: number;  //iterators
        var parts: RegExpExecArray | null;  // to store string splitted

        //process line by line
        var line = assemblySource.split("\n")
        var ret = ""
        //loop thru all lines
    ...
    private static isFunction(text: string) {
    ...
    private static toSpan(text: string, classname: string) {
